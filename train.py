...
class GPT(nn.Module):
    ...
    def _compute_window_sizes(self, config):
        ...
        char_to_window_odd = {"L": (long_window, 0), "S": (320, 0)}
        window_sizes = []
        for layer_idx in range(config.n_layer):
            char = pattern[layer_idx % len(pattern)]
            if layer_idx % 2 == 0:
                window_sizes.append(char_to_window_even[char])
            else:
                window_sizes.append(char_to_window_odd[char])
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        param_groups = [
            ...
            dict(kind="adamw", params=value_embeds_params, lr=1.3 * dmodel_lr_scale, betas=adam_betas, eps=1e-10, weight_decay=0.003),
...
class MuonAdamW(torch.optim.Optimizer):
    ...
    def _step_muon(self, group):
        ...
        self._muon_lr_t.fill_(group["lr"] * max(1.0, shape[-2] / shape[-1])**0.4)
...
EMBEDDING_LR = 1.15      # learning rate for token embeddings (Adam)
