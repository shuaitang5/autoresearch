...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        if ve is not None:
            ...
            gate = 2.5 * torch.sigmoid(self.ve_gate(x[..., :self.ve_gate_channels]))
        ...
        y = fa3.flash_attn_func(q, k, v, causal=True, window_size=window_size, softmax_scale=0.10)
...
class GPT(nn.Module):
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        param_groups = [
            dict(kind='adamw', params=lm_head_params, lr=unembedding_lr * dmodel_lr_scale, betas=adam_betas, eps=1e-10, weight_decay=0.018),
            dict(kind='adamw', params=embedding_params, lr=embedding_lr * dmodel_lr_scale, betas=adam_betas, eps=1e-10, weight_decay=0.0),
            dict(kind="adamw", params=value_embeds_params, lr=embedding_lr * 1.5 * dmodel_lr_scale, betas=adam_betas, eps=1e-10, weight_decay=0.005),
            dict(kind='adamw', params=resid_params, lr=scalar_lr * 0.02, betas=adam_betas, eps=1e-10, weight_decay=0.0),
            dict(kind='adamw', params=x0_params, lr=scalar_lr, betas=(0.8, 0.95), eps=1e-10, weight_decay=0.0),
        ]
        for shape in sorted({p.shape for p in matrix_params}):
            group_params = [p for p in matrix_params if p.shape == shape]
            param_groups.append(dict(
                kind='muon', params=group_params, lr=matrix_lr,
                momentum=0.95, ns_steps=5, beta2=0.84, weight_decay=weight_decay,
            ))
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        if targets is not None:
            ...
            weights = torch.linspace(0.8, 1.2, T, device=logits.device).repeat(B)
...
while True:
    ...
    if progress > 1.0 - WARMDOWN_RATIO:
        ...
        muon_momentum = muon_momentum * (0.92 + 0.08 * cooldown_frac)  # decay from 0.95 to 0.9025
