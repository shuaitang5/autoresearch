...
class Block(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.mlp = MLP(config, expansion=16 if is_last else 4)
...
class GPT(nn.Module):
    ...
    def init_weights(self):
        ...
        for block in self.transformer.h:
            ...
            # Scale MLP c_fc init by fan_in/fan_out ratio
            fan_in = block.mlp.c_fc.weight.shape[1]
            fan_out = block.mlp.c_fc.weight.shape[0]
            s_mlp = (6.0 / (fan_in + fan_out)) ** 0.5
        ...
        for block in self.transformer.h:
            if block.attn.ve_gate is not None:
                torch.nn.init.normal_(block.attn.ve_gate.weight, std=0.01)
        # Rotary embeddings
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        for shape in sorted({p.shape for p in matrix_params}):
            ...
            param_groups.append(dict(
                ...
                momentum=0.95, ns_steps=5, beta2=0.93, weight_decay=weight_decay,
            ))
...
FINAL_LR_FRAC = 0.020     # final LR as fraction of initial
...
def get_weight_decay(progress):
    return WEIGHT_DECAY * max(0.5 * (1 + math.cos(math.pi * progress)), 0.28)
