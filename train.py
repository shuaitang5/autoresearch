...
class MLP(nn.Module):
    def __init__(self, config, mlp_ratio=5.5):
        super().__init__()
        mlp_dim = int(mlp_ratio * config.n_embd)
...
class GPT(nn.Module):
    ...
    def _compute_window_sizes(self, config):
        ...
        window_sizes[-1] = (960, 0)  # 1024 context for last layer (saves compute)
        return window_sizes
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=value_embeds_params, lr=0.39, betas=adam_betas, eps=1e-10, weight_decay=0.005),
...
def get_lr_multiplier(progress):
    if progress < WARMUP_RATIO:
        ...
        cosine = 0.5 * (1.0 + math.cos(math.pi * (1.0 - cooldown)))
